...
class KnowWhereDB:
    ...
    def __init__(self, db_url: str | None = None, user_id: str = "default"):
        self.db_url = db_url or DEFAULT_DB_URL
        self.user_id = user_id or os.environ.get("KNOWWHERE_USER_ID", "default")
    ...
    def upsert_summary(
        ...
        with self.conn.cursor() as cur:
            cur.execute(
                """INSERT INTO summaries (session_id, project, summary_text, embedding, tier, anchor_id, user_id)
                   ...
                   ON CONFLICT (session_id) DO UPDATE SET
                       ...
                       updated_at = NOW()
                   RETURNING id""",
                (session_id, project, summary_text, embedding, tier, anchor_id, uid),
            )
    ...
    def search_hybrid(
        ...
        *,
        ...
        """Hybrid search: UCB-weighted embedding + PostgreSQL full-text keyword boost.

        query_text is split into words (>2 chars), OR-combined into a tsquery,
        and fused with cosine similarity. Falls back to pure embedding when
        query_text is empty or has no usable keywords.
        """
        uid = user_id or self.user_id
        # Extract usable keywords
        keywords = [w.strip().lower() for w in query_text.split()
                    if len(w.strip()) > 2]
        if not keywords:
            return self.search_relevant(
                query_embedding, project=project, top_k=top_k,
                min_score=min_score, ucb_weight=ucb_weight,
                record_access=record_access, user_id=uid,
            )

        ts_query_str = " | ".join(keywords)
        # 1 - kw  => embedding weight;  LEAST caps keyword boost at 1.0
        emb_w = 1.0 - keyword_weight
        boost_factor = 3.0  # amplify ts_rank so a decent match approaches 1.0
        ...
        params: list[Any] = [
            uid,                       # $0  user_id filter
            query_embedding,           # $1  embedding <=>
            ts_query_str,              # $2  ts_rank arg 1
            query_embedding,           # $3  embedding <=> (second use)
            emb_w,                     # $4  embedding weight
            boost_factor,              # $5  LEAST multiplier
            ts_query_str,              # $6  ts_rank arg 2
            keyword_weight,            # $7  keyword weight
            ucb_weight,                # $8  UCB weight
            query_embedding,           # $9  WHERE embedding <=>
            min_score,                 # $10 WHERE min_score
            top_k,                     # $11 LIMIT
        ]
    ...
    def get_debuts(self, project: str | None = None, limit: int = 5, user_id: str | None = None) -> list[dict]:
        """Get summaries that have never been injected (debut_seen=FALSE)."""
        uid = user_id or self.user_id
        if project:
            query = "WHERE debut_seen = FALSE AND user_id = %s AND project = %s ORDER BY created_at DESC LIMIT %s"
            params = [uid, project, limit]
        else:
            query = "WHERE debut_seen = FALSE AND user_id = %s ORDER BY created_at DESC LIMIT %s"
            params = [uid, limit]
    ...
    def insert_source(
        ...
        with self.conn.cursor() as cur:
            cur.execute(
                """INSERT INTO sources (session_id, full_text, content_hash, char_count, user_id, metadata)
                   VALUES (%s, %s, %s, %s, %s, %s)
                   ON CONFLICT (content_hash) DO UPDATE
                       SET metadata = sources.metadata || EXCLUDED.metadata,
                           user_id = EXCLUDED.user_id
                   RETURNING id""",
                (session_id, full_text, content_hash, len(full_text), uid,
    ...
    def health_check(self, user_id: str | None = None) -> dict:
        """Quick database health check, optionally scoped to a user."""
        uid = user_id or self.user_id
        with self.conn.cursor() as cur:
            cur.execute("SELECT COUNT(*) FROM summaries WHERE user_id = %s", (uid,))
            summary_count = cur.fetchone()[0]
            cur.execute("SELECT COUNT(*) FROM sources WHERE user_id = %s", (uid,))
            source_count = cur.fetchone()[0]
            cur.execute("SELECT COUNT(*) FROM vector_index")
            index_count = cur.fetchone()[0]
            cur.execute(
                "SELECT COUNT(*) FROM summaries "
                "WHERE debut_seen = FALSE AND user_id = %s", (uid,)
            )
